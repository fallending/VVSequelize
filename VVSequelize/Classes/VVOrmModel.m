...
/**
 ...
 通常iOS App内嵌数据库单表的数据量不会达到rowid最大值，所以此处取 max(rowid)+1 作为自动主键值，提供给外部使用。
 */
- (NSUInteger)nextAutoPrimaryKey{
    id mrowid = [self max:@"rowid"];
    if(!mrowid || [mrowid isKindOfClass:NSNull.class]) return 1;
    return [mrowid unsignedIntegerValue] + 1;
}
