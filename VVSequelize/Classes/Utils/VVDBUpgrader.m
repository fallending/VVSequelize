...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self setLastVersionGetter:^NSString *{
            return [[NSUserDefaults standardUserDefaults] stringForKey:VVDBUpgraderLastVersionKey];
        }];
        [self setLastVersionSetter:^(NSString *version) {
            if (version.length > 0) {
                [[NSUserDefaults standardUserDefaults] setObject:version forKey:VVDBUpgraderLastVersionKey];
            } else {
                [[NSUserDefaults standardUserDefaults] removeObjectForKey:VVDBUpgraderLastVersionKey];
            }
